...
1. Run the ordinary note audit first and preserve its timestamped report as the before evidence.
2. Create a new repaired working folder inside that run. Preserve the note's relative directory structure and copy the local styles, images, attachments, and linked notes required for it to work. Never overwrite the supplied source unless the user separately and explicitly requests in-place editing.
3. Apply `--fix-safe` metadata fixes, then inspect the report and source. Use Codex to make the smallest justified repairs for structure, navigation, portable paths, mobile readability, accessibility markup, and unsafe HTML behavior. Repair a missing or case-mismatched path only when the intended local target can be discovered. Remove or change a script or remote dependency only when its purpose is understood and the note remains functional.
