...
6. Deliver the before `report.html`, repaired entry HTML or folder, after `report.html`, bilingual comparison artifact, a concise change summary, and every detected unresolved finding. Call the result directly usable only when the after check has no errors, the comparison has no new/worsened/unverified findings, all required local assets were available for verification, and any remaining warnings are disclosed rather than hidden.
