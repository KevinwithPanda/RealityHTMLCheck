---
name: realitycheck
description: Check, repair, verify, and create evidence-bound passive-static publish packages from local HTML notes or folders, including AI-generated and exported notes; bind a verified capsule to its authorized live deployment; and preview, audit, safely stress-test, fix, and verify a developer-controlled Web app with deterministic real-browser scenarios and before/after proof. Use when Codex receives an HTML note/export, the user asks whether it is correct, portable, shareable, deployed as expected, or ready to publish, local resources are broken, or a localhost or authorized UI needs evidence-first QA and repair.
---
...
- Treat an online imported export ZIP and "verified safe-metadata folder ZIP" as a local, bounded ZIP32 intake plus a cumulative recheck of only those same three fixes over every imported file, not as a complete agentic repair or proof that missing/remote resources were supplied. The browser records the exact source ZIP, extracted-content, analysis-candidate, and output-archive identities separately; encrypted, ZIP64, unsafe, sensitive, non-regular, damaged, or oversized archives fail closed. When the user wants structure, paths, accessibility, scripts, or content repaired, use the `note repair` working-copy workflow instead of treating that ZIP as final.
- `note <file-or-directory> --exclude-html <portable-glob>`: when a mixed note folder contains deliberate archives or drafts outside the sharing target, repeat this option for the narrowest reviewed paths. Exclusions remove only per-file HTML checks; keep matched files in package and cross-note integrity, disclose the rules and matches, and never add an exclusion merely to make a gate pass. Against a baseline, treat a newly excluded previously checked scope as error-level unverified until the user intentionally accepts a new baseline.
- `note <file-or-directory> --baseline <prior-report.json>`: when the user asks what changed since an earlier note check or wants a regression-only publishing gate, compare against an immutable prior bundle. Report new, resolved, worsened, persistent, and unverified findings. Gate only new/worsened/unverified findings at the requested error/warning level; never call a finding resolved when an HTML file disappeared, package discovery shrank, or current coverage was truncated.
...
- `note publish <html-file|directory|zip>`: when the user asks for a deployable/shareable static HTML package, a verified publish ZIP, or a host handoff, run `scripts/audit.mjs note publish` after any authorized agentic repair when the checked-out package dependencies are available. Preserve the original user-facing package name with `--name <original-name>` when publishing a generic working directory; prepared `repaired/` outputs also recover the bounded original name from their sibling report unless explicitly overridden. A standalone installed Skill does not bundle Playwright Core: if its capability probe reports that dependency unavailable, use the version-pinned GitHub package `npx --yes --package="github:KevinwithPanda/RealityHTMLCheck#v0.12.0" realityhtmlcheck publish ...` so the same Skill invocation still completes end to end; do not downgrade silently to static-only proof. This local command may apply only the declared safe metadata changes and uniquely resolvable HTML/CSS case/backslash path fixes inside frozen output bytes; it never overwrites the source, uploads, deploys, signs in, or controls a cloud account. It must read back the final ZIP and complete its required passive Chromium scenarios before returning a `*.realitycheck-publish.zip`; otherwise return only the explicitly named working copy and blockers. Treat interactive scripts, forms, frames, server runtimes, remote runtime dependencies, ambiguous entry pages, missing files, sensitive paths, symlinks, incomplete coverage, or browser failure as blockers rather than silently deleting behavior. Use `--entry <exact-relative-html>` only when the user or package structure establishes the intended homepage; never guess among several pages.
- `publish Action`: when the user asks to make an export or GitHub pipeline continuously produce verified capsules, use the Composite Action with `kind: publish` and read the publish-Action section in [references/html-notes.md](references/html-notes.md). It must upload only the parser-validated exact run, preserve a blocked working copy before failing the gate, and never request deployment permissions. Explicitly disclose that `upload-artifact: true` transfers the complete HTML/site bytes to GitHub Artifact storage; use `false` when that transfer is not intended.
- `materialize <publish-run> --output <new-directory>`: when a user or downstream host needs an extracted static directory, materialize only a complete publish-ready run. The command must validate the run, receipt, final browser proof, ZIP and sidecar again; create a separate absent directory atomically; preserve every archive byte including safe hidden files; keep its stage receipt outside the public tree; and never substitute the original source folder for the verified ZIP. A standalone installed Skill does not bundle the validator dependencies: when the checked-out runtime capability probe cannot resolve them, use the pinned `npx --yes --package="github:KevinwithPanda/RealityHTMLCheck#v0.12.0" realityhtmlcheck materialize ...` path rather than attempting a partial extraction.
- `verify-deploy <publish-run> <http(s)-base-url/> --allow-remote`: when the user asks whether a manual or CI deployment actually serves the verified capsule, or supplies an authorized live URL after upload, read the live-deployment section in [references/html-notes.md](references/html-notes.md). Require the complete publish run rather than an unbound loose ZIP. Contact only the declared same-origin base path with bounded credential-free GETs; compare every declared servable file by size and SHA-256; skip only the `.nojekyll` platform marker; then run the JavaScript-disabled live desktop, 375px mobile, page, fragment, and unexpected-network proof. Preserve `LIVE MATCH`, `LIVE TRANSFORMED / REVIEW`, `LIVE BROKEN`, and `UNVERIFIED` as distinct decisions. A public green decision requires HTTPS, complete byte coverage, and all live browser scenarios; HTTP can produce diagnostic `UNVERIFIED` evidence only. A standalone installed Skill does not bundle Ajv or Playwright Core: if its capability probe cannot resolve them, use the pinned `npx --yes --package="github:KevinwithPanda/RealityHTMLCheck#v0.12.0" realityhtmlcheck verify-deploy ...` path. The result is a point-in-time receipt, never proof of account ownership, permanent availability, global CDN consistency, or automatic rollback. This action verifies but never deploys, changes DNS, manages a custom domain, or logs into a host.
...
5. Rerun the same note audit against the repaired working folder with the immutable before `report.json` passed as `--baseline`. Retain the generated `comparison.html` and `comparison.json`; do not infer resolution from score alone. When visual usability is requested and a browser adapter is available, preview the repaired entry HTML without allowing its scripts or remote resources to escape the safety boundary.
6. Deliver the before `report.html`, repaired entry HTML or folder, after `report.html`, bilingual comparison artifact, a concise change summary, and every unresolved finding. Call the result directly usable only when the after check has no errors, the comparison has no new/worsened/unverified findings, all required local assets were available for verification, and any remaining warnings are disclosed rather than hidden.
...
- every publish-ready note archive has a verified final read-back, deploy-content identity, final-archive browser proof, SHA-256 sidecar, and host-specific decision; incomplete publish coverage is named and returned only as a working copy.
- every claimed live deployment match has a validated capsule source, complete per-path hosted-byte evidence, a passed live browser proof, a bilingual time-stamped deployment receipt, explicit screenshot/retention disclosure, and no unsupported permanence or deployment claim.
