...
- Treat an online "verified safe-metadata folder ZIP" as a cumulative recheck of only those same three fixes over every browser-selected file, not as a complete agentic repair or proof that missing/remote resources were supplied. When the user wants structure, paths, accessibility, scripts, or content repaired, use the `note repair` working-copy workflow instead of treating that ZIP as final.
