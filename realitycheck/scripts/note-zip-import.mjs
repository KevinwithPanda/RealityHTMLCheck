import { digestZipSource, validatePortableZipEntryPath } from "./note-zip.mjs";
...
function inflatedBytes(value) {
  if (value instanceof ArrayBuffer) return new Uint8Array(value);
  if (ArrayBuffer.isView(value)) return new Uint8Array(value.buffer, value.byteOffset, value.byteLength);
  throw new TypeError("The DEFLATE adapter must return an ArrayBuffer or byte view");
}

function verifyInflatedSize(output, expectedSize) {
  if (output.byteLength > expectedSize) throw new Error("DEFLATE output exceeds the size declared by the ZIP central directory");
  if (output.byteLength !== expectedSize) throw new Error("DEFLATE output size differs from the ZIP central directory");
  return output.slice();
}

async function inflateRawBounded(compressed, expectedSize, signal, inflateRaw) {
  throwIfAborted(signal);
  if (inflateRaw !== undefined) {
    if (typeof inflateRaw !== "function") throw new TypeError("inflateRaw must be a function when provided");
    const output = inflatedBytes(await inflateRaw(compressed, { expectedSize, signal }));
    throwIfAborted(signal);
    return verifyInflatedSize(output, expectedSize);
  }
  if (typeof DecompressionStream !== "function") {
    throw new Error("This runtime cannot decompress DEFLATE ZIP entries without an inflateRaw adapter");
  }
  let stream;
  try {
...
/**
 * Parse one bounded ZIP into immutable byte entries without requiring DOM File.
 * Node callers should provide an inflateRaw adapter; browsers use the native
 * DecompressionStream path when one is available.
 */
export async function extractHtmlNoteZip(source, { signal, limits: configured, inflateRaw } = {}) {
  if (!source || typeof source !== "object" || !Number.isSafeInteger(source.size) || source.size < 0 || typeof source.arrayBuffer !== "function") throw new TypeError("ZIP import requires one file-like byte source");
  ...
  for (const entry of pathPlan.entries.sort((left, right) => compareText(left.path, right.path))) {
    ...
    } else content = await inflateRawBounded(compressed, entry.uncompressedSize, signal, inflateRaw);
  ...
  const importContract = {
    ...
    entries: imported.map((entry) => ({ path: entry.path, bytes: entry.size, crc32Hex: entry.crc32Hex, sha256: entry.sha256 })),
  };
  const importDigest = await digestZipSource({ bytes: new TextEncoder().encode(JSON.stringify(importContract)) }, { signal });
  return {
    kind: "html-note-zip-extract",
    schemaVersion: "1",
    source,
    entries: imported,
    manifest: {
      ...
      methods,
      entries: imported.map(({ data: _data, type: _type, size, ...entry }) => ({ ...entry, bytes: size })),
      limits,
    },
  };
}

/** Browser adapter preserving the v0.9 File-based API. */
export async function importHtmlNoteZip(source, options = {}) {
  const extracted = await extractHtmlNoteZip(source, options);
  if (typeof File !== "function") {
    throw new Error("DOM File is unavailable; use extractHtmlNoteZip or the Node file adapter instead");
  }
  const imported = extracted.entries.map((entry) => {
    const filename = entry.path.split("/").at(-1);
    const file = new File([entry.data], filename, { type: entry.type, lastModified: 0 });
    return { ...entry, file };
  });
  return {
