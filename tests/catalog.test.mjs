...
test("artifact catalog indexes valid audits and proofs with portable bilingual links", () => {
  ...
  try {
    ...
    const catalog = buildArtifactCatalog([pageDirectory, proofDirectory, invalidDirectory, signed.jsonPath], output, { now: new Date("2026-08-01T16:00:00Z") });
    assert.equal(catalog.summary.artifacts, 3);
    ...
    assert.equal(catalog.summary.failing, 1);
