...
test("zero-install note checker exposes a private file and folder workflow", () => {
  ...
  assert.match(html, /id="copy-skill-install"/);
  assert.match(html, /id="copy-skill-repair"/);
  ...
  assert.match(html, /What happens after the check\?/);
  assert.match(html, /bind the live HTTPS URL to its exact bytes/);
  assert.match(html, /script type="module" src="note-checker\.js\?v=0\.13\.0"/);
  assert.match(html, /<link rel="manifest" href="site\.webmanifest">/);
  assert.match(html, /Online ZIP repair is a safe-metadata working copy, not a publish or live-deployment verdict/);
  assert.match(html, /v0\.13 local command or Action/);
