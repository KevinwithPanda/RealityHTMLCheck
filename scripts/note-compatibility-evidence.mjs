...
function analyzeFixture(definition) {
  ...
  let reports = htmlPaths.map((path) => analyzeHtmlNote({
    path,
    html: readFileSync(join(directory, path), "utf8"),
    knownFiles,
  }));
  const packageFindings = analyzeNotePackage({ entries, knownFiles });
  if (packageFindings.length) reports[0] = mergePackageFindings(reports[0], packageFindings);
  const summary = summarizeNoteReports(reports);
  const detectedRules = [...new Set(reports.flatMap((report) => report.findings.map((finding) => finding.ruleId)))].sort();
